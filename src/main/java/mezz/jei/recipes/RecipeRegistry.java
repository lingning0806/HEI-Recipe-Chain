...
public class RecipeRegistry implements IRecipeRegistry {
    ...
    public IRecipeTransferHandler getRecipeTransferHandler(Container container, IRecipeCategory recipeCategory) {
        ...
        if (recipeTransferHandler != null) {
            return adaptAe2(container, recipeCategory, recipeTransferHandler);
        }

        IRecipeTransferHandler universal = recipeTransferHandlers.get(containerClass, Constants.UNIVERSAL_RECIPE_TRANSFER_UID);
        if (universal != null) return adaptAe2(container, recipeCategory, universal);
        // AA's portable workbench only overrides reachability; never generalize to unknown containers.
        if (containerClass.getName().equals("de.ellpeck.actuallyadditions.mod.util.compat.CompatUtil$1") &&
            container instanceof net.minecraft.inventory.ContainerWorkbench && container.inventorySlots.size() == 46 &&
            container.getSlot(0) instanceof net.minecraft.inventory.SlotCrafting) {
            return recipeTransferHandlers.get(net.minecraft.inventory.ContainerWorkbench.class, recipeCategory.getUid());
        }
        return null;
    }

    private IRecipeTransferHandler adaptAe2(Container container, IRecipeCategory category, IRecipeTransferHandler handler) {
        if (mezz.jei.nova.ae2.Ae2Terminal.supports(container) && "minecraft.crafting".equals(category.getUid()) &&
            mezz.jei.nova.ae2.Ae2Terminal.acceptsHandler(handler.getClass().getName()))
            return new mezz.jei.nova.ae2.Ae2CraftingHandler(handler);
        return handler;
    }
