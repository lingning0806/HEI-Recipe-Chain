...
public class RecipeBookmarkItem<I> extends BookmarkItem<I> {
    ...
    public int savedViewMode;
    private net.minecraft.nbt.NBTTagList savedMaterials = new net.minecraft.nbt.NBTTagList();
    // Session-only concrete input choices per operation; never persisted in bookmarks.
    public java.util.Map<String,Long> executionInputs;
    public mezz.jei.nova.InfusionDemand infusion;
    public mezz.jei.nova.InputUsage usage = mezz.jei.nova.InputUsage.CONSUMED;
    ...
    public RecipeBookmarkItem(RecipeBookmarkItem<I> other) {
        ...
        this.executionInputs = other.executionInputs;
        this.selfOutputAmount = other.selfOutputAmount;
        this.recipe = other.recipe;
        this.usage = other.usage;
        this.infusion = other.infusion;
        this.category = other.category;
        this.inputs = other.inputs;
        this.savedViewMode = other.savedViewMode;
        this.savedMaterials = (net.minecraft.nbt.NBTTagList) other.savedMaterials.copy();
    ...
    public void populateWith(IRecipeWrapper recipe, IRecipeCategory<?> category) {
        ...
        mezz.jei.nova.TransposerSelection.apply(recipe, ingredient, category.getUid(), ingredients);
        ...
        mezz.jei.nova.preferences.Preferences.apply(this, savedMaterials);
        savedMaterials = mezz.jei.nova.preferences.Preferences.snapshot(this);
        ...
        if (recipe instanceof mezz.jei.nova.RecipeInputUsageProvider) {
            for (RecipeBookmarkItem<?> input : inputs) {
                input.usage = ((mezz.jei.nova.RecipeInputUsageProvider) recipe).getInputUsage(input.aliases);
                if (input.usage == null) input.usage = mezz.jei.nova.InputUsage.CONSUMED;
            }
        }
        mezz.jei.nova.InfusionDemand.adapt(recipe, inputs);
        java.util.Map<String, List<RecipeBookmarkItem<?>>> displayGroups = new java.util.LinkedHashMap<>();
        for (RecipeBookmarkItem<?> input : inputs) {
            String id = input.infusion != null ? input.infusion.key : Internal.getIngredientRegistry().getUniqueId(input.aliases.get(0));
            displayGroups.computeIfAbsent(id, ignored -> new ObjectArrayList<>()).add(input);
        }
        inputDummyItems = new ObjectArrayList<>();
        for (List<RecipeBookmarkItem<?>> sameIcon : displayGroups.values()) {
            RecipeBookmarkItem<?> first = sameIcon.get(0);
            if (first.infusion != null) inputDummyItems.add(new mezz.jei.bookmarks.InfusionBookmarkItem(first.ingredient, getGroup(), () -> Math.multiplyExact(first.amount,getMultiplier()),first.infusion));
            else inputDummyItems.add(new mezz.jei.bookmarks.RecipeInputBookmarkItem(this, sameIcon));
        }
    }
    ...
    public boolean startsNewRow() {
        if(getGroup() instanceof RecipeBookmarkGroup && ((RecipeBookmarkGroup)getGroup()).viewMode!=0) return false;
    ...
    public long getMultiplier() {
        ...
        return mezz.jei.nova.ChainMath.batches(amount, outputAmount);
    }
    ...
    public long getDisplayAmount() {
        ...
        return Math.multiplyExact(outputAmount, getMultiplier());
    }

    @Override
    public boolean deserialize(NBTTagCompound serialized) {
        super.deserialize(serialized);
        this.savedViewMode=Math.max(0,Math.min(2,serialized.getInteger("novaView")));
        this.selfOutputAmount = serialized.getLong("selfOutputAmount");
        this.savedMaterials = serialized.getTagList("novaMaterials",10);
    ...
    public String serialize() {
        ...
        tag.setInteger("novaView",getGroup() instanceof RecipeBookmarkGroup?((RecipeBookmarkGroup)getGroup()).viewMode:savedViewMode);
        tag.setTag("novaMaterials", savedMaterials);
