...
public class BookmarkList implements IIngredientGridSource {
    ...
    private final Deque<List<BookmarkGroup>> undoHistory = new ArrayDeque<>();

    private List<BookmarkGroup> snapshot() {
        List<BookmarkGroup> result = new ArrayList<>();
        for (BookmarkGroup group : list) {
            if (group instanceof RecipeBookmarkGroup) result.add(((RecipeBookmarkGroup) group).copyForEdit());
            else {
                BookmarkGroup copy = new BookmarkGroup(group.id);
                for (BookmarkItem<?> item : group.items) {
                    BookmarkItem<?> cloned = item.copy();
                    cloned.amount = item.amount;
    ...
    private void commit(List<BookmarkGroup> replacement) {
        undoHistory.push(snapshot());
        while (undoHistory.size() > 16) undoHistory.removeLast();
        list.clear();
        for (BookmarkGroup group : replacement) if (!group.getItemsInternal().isEmpty()) list.add(group);
        notifyListenersOfChange();
        saveBookmarks();
    }

    public boolean undoRecipeEdit() {
        if (undoHistory.isEmpty()) return false;
        list.clear(); list.addAll(undoHistory.pop());
        notifyListenersOfChange(); saveBookmarks();
        return true;
    }

    public boolean splitRecipeGroup(RecipeBookmarkGroup group) {
        if (!list.contains(group) || group.getItemsInternal().size() < 2) return false;
        List<BookmarkGroup> replacement = new ArrayList<>(list);
        int index = replacement.indexOf(group);
        replacement.remove(index);
        replacement.addAll(index, group.split(this::nextId));
        commit(replacement);
        return true;
    }

    public boolean changeRecipeAmount(mezz.jei.autocrafting.RecipeBookmarkItem<?> item, long delta) {
        if (!(item.getGroup() instanceof RecipeBookmarkGroup)) return false;
        RecipeBookmarkGroup group = (RecipeBookmarkGroup) item.getGroup();
        if (!list.contains(group)) return false;
        int index = group.getItemsInternal().indexOf(item);
        if (index < 0) return false;
        RecipeBookmarkGroup updated = group.copyForEdit();
        mezz.jei.autocrafting.RecipeBookmarkItem<?> copy = (mezz.jei.autocrafting.RecipeBookmarkItem<?>) updated.getItemsInternal().get(index);
        copy.selfOutputAmount = Math.max(0, Math.addExact(copy.selfOutputAmount, delta));
        updated.update();
        List<BookmarkGroup> replacement = new ArrayList<>(list);
        replacement.set(replacement.indexOf(group), updated);
        commit(replacement);
        return true;
    }
    ...
    private void removeItemFromGroup(BookmarkGroup group, BookmarkItem<?> item) {
        ...
        if (group instanceof RecipeBookmarkGroup) {
            replacement.set(replacement.indexOf(group), ((RecipeBookmarkGroup) group).without(item));
            commit(replacement);
        } else {
            group.removeItem(item);
            if (group.items.isEmpty()) list.remove(group);
            undoHistory.clear();
            notifyListenersOfChange(); saveBookmarks();
        }
    }
    ...
    public void saveBookmarks() {
        ...
        if (file != null) {
            ...
            try {
                java.nio.file.Path destination = file.toPath().toAbsolutePath();
                temporary = java.nio.file.Files.createTempFile(destination.getParent(), "hei-bookmarks-", ".tmp");
                java.nio.file.Files.write(temporary, strings, java.nio.charset.StandardCharsets.UTF_8);
                try {
    ...
    /** Range grouping: keep the first selected position; rebuild and save only once. */
    public void mergeRecipeGroups(List<Integer> ids) {
        if (ids.size() < 2) return;
        List<RecipeBookmarkGroup> selected = new ArrayList<>();
        for (BookmarkGroup group : list) if (ids.contains(group.id)) {
            if (!(group instanceof RecipeBookmarkGroup)) throw new IllegalArgumentException("Non-recipe group");
            selected.add((RecipeBookmarkGroup) group);
        }
        if (selected.size() != ids.size()) throw new IllegalArgumentException("Groups changed during drag");
        RecipeBookmarkGroup merged = selected.get(0);
        for (int i=1;i<selected.size();i++) merged = merged.mergedWith(selected.get(i));
        List<BookmarkGroup> replacement = new ArrayList<>(list);
        int anchor = replacement.indexOf(selected.get(0));
        replacement.removeAll(selected);
        replacement.add(anchor, merged);
        commit(replacement);
    }

    public void mergeRecipeGroups(mezz.jei.autocrafting.RecipeBookmarkGroup source,
                                  mezz.jei.autocrafting.RecipeBookmarkGroup target) {
        if (source == target || !list.contains(source) || !list.contains(target)) return;
        mezz.jei.autocrafting.RecipeBookmarkGroup merged = target.mergedWith(source);
        List<BookmarkGroup> replacement = new ArrayList<>(list);
        replacement.set(replacement.indexOf(target), merged);
