...
public class BookmarkGridWithNavigation implements IShowsRecipeFocuses, IMouseHandler, IGhostIngredientDragSource {
    ...
    public void drawTooltips(Minecraft minecraft, int mouseX, int mouseY) {
        if (groupOrganizer.isDragging()) {
            groupOrganizer.drawTooltips(minecraft, mouseX, mouseY);
            return;
        }
    ...
    public boolean handleMouseScrolled(int mouseX, int mouseY, int scrollDelta) {
        ...
        if ((Keyboard.isKeyDown(Keyboard.KEY_LCONTROL) || Keyboard.isKeyDown(Keyboard.KEY_RCONTROL)) && element != null) {
            ...
            mezz.jei.nova.CraftDiagnostics.log("quantity input type="+item.getClass().getSimpleName()+" before="+item.getDisplayAmount()+" wheel="+scrollDelta);
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT) || Keyboard.isKeyDown(Keyboard.KEY_RSHIFT)) {
                if (item.ingredient instanceof ItemStack) {
                    int stackSize = ((ItemStack) item.ingredient).getMaxStackSize();
                    changeBookmarkAmount(item, scrollDelta < 0 ? -stackSize : stackSize);
                } else if (item.ingredient instanceof FluidStack) {
                    changeBookmarkAmount(item, scrollDelta < 0 ? -1000 : 1000);
                } else {
                    changeBookmarkAmount(item, scrollDelta < 0 ? -1 : 1);
                }
            } else {
                changeBookmarkAmount(item, scrollDelta < 0 ? -1 : 1);
            }
            mezz.jei.nova.CraftDiagnostics.log("quantity after="+item.getDisplayAmount());
    ...
    private void changeBookmarkAmount(mezz.jei.bookmarks.BookmarkItem<?> item, long delta) {
        if (mezz.jei.Internal.getRuntime().getAutocraftingHandler().isActive()) return;
        if (item instanceof mezz.jei.autocrafting.RecipeBookmarkItem) {
            mezz.jei.Internal.getBookmarkList().changeRecipeAmount((mezz.jei.autocrafting.RecipeBookmarkItem<?>) item, delta);
        } else item.changeAmount(delta);
    }
