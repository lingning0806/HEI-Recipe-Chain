...
public class RecipesGui extends GuiScreen implements IRecipesGui, IShowsRecipeFocuses, IRecipeLogicStateListener {
	...
	private boolean handleKeybinds(int eventKey) {
        if (KeyBindings.crafting.isActiveAndMatches(eventKey)) {
            if (org.lwjgl.input.Keyboard.isRepeatEvent()) return true;
            for (RecipeLayout layout : recipeLayouts) {
                mezz.jei.gui.ingredients.GuiIngredient<?> slot = layout.getGuiIngredientUnderMouse(MouseHelper.getX(), MouseHelper.getY());
                if (slot != null && !slot.isInput()) return mezz.jei.nova.QuickCraft.run(this, layout, isCtrlKeyDown());
            }
        }
        if (KeyBindings.bookmarkRecipe.isActiveAndMatches(eventKey)) {
            if (org.lwjgl.input.Keyboard.isRepeatEvent()) return true;
            for (RecipeLayout layout : recipeLayouts) {
                if (layout.isMouseOver(MouseHelper.getX(), MouseHelper.getY())) {
                    layout.getRecipeBookmarkButton().bookmarkSingle();
                    return true;
                }
            }
        }
        if (KeyBindings.planRecipeTree.isActiveAndMatches(eventKey)) {
            if (org.lwjgl.input.Keyboard.isRepeatEvent()) return true;
            for (RecipeLayout layout : recipeLayouts) {
                if (layout.isMouseOver(MouseHelper.getX(), MouseHelper.getY())) {
                    layout.getRecipeBookmarkButton().openPlanner(this);
                    return true;
                }
            }
        }
