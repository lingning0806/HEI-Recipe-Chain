...
public class GhostIngredientDragManager {
    ...
    public boolean isDragging(){return ghostIngredientDrag!=null;}
    public boolean release(int x,int y) {
        if(ghostIngredientDrag!=null && Math.abs(x-dragStartX)+Math.abs(y-dragStartY)>4 && ghostIngredientDrag.onClick(x,y)) {
            ghostIngredientDrag=null;return true;
        }
        return false;
    }
    ...
    public <T extends GuiScreen, V> boolean handleClickGhostIngredient(IGhostIngredientHandler<T> handler, T currentScreen, IClickedIngredient<V> clicked) {
        ...
        if(ingredient instanceof mezz.jei.bookmarks.BookmarkItem) {
            IClickedIngredient<?> unwrapped=mezz.jei.input.ClickedIngredient.create(((mezz.jei.bookmarks.BookmarkItem<?>)ingredient).ingredient,clicked.getArea());
            boolean handled=handleClickGhostIngredient(handler,currentScreen,unwrapped);if(handled)clicked.onClickHandled();return handled;
        }
        ...
        if (!targets.isEmpty()) {
            ...
            dragStartX=mezz.jei.input.MouseHelper.getX();dragStartY=mezz.jei.input.MouseHelper.getY();
